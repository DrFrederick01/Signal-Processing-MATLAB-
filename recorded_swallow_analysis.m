%% Loads audio files in current directory
directory = [pwd '/audio/'];
...
for i = 1:length(files)
    ...
    grid on;
    saveas(gcf, [pwd sprintf('/output/recorded/time-domain-%u.png', i)]);
    ...
    grid on;
    saveas(gcf, [pwd sprintf('/output/recorded/frequency-spectrum-%u.png', i)]);
    ...
    legend(labels, 'Location', 'best');
    saveas(gcf, [pwd sprintf('/output/recorded/power-and-snr-bar-%u.png', i)]);

    %% Redo frequency analysis
    % For consistency, we downsample all recordings in this analysis by a
    % factor of 400
    audioData_down = downsample(audioData, 400);
    Fs_down = Fs / 400;
    t_down = (0:length(audioData_down)-1) / Fs_down;
    ...
    grid on;
    saveas(gcf, [pwd sprintf('/output/recorded/downsampled-frequency-spectrum-%u.png', i)]);
    ...
    saveas(gcf, [pwd sprintf('/output/recorded/downsampled-%u.png', i)]);
end
